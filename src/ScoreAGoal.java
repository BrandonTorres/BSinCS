...
public class ScoreAGoal extends Card{
	public ScoreAGoal(String Name, String Path, int Version,int[]CR){
		CardName=Name;
		imagePath=Path;
	...
	public void PlayCard(Player currentPlayer,int ChipChoice,ArrayList<Card> Discard,ArrayList<Card> Deck,ArrayList<Room> RoomList){
		...
		int[] CorrectRooms=new int[]{1};
		
		for(int i=0; i<CorrectRooms.length;i++)
			if(TempPlayer.GetPlayerRoom().GetRoomID()==CorrectRooms[i]&&TempPlayer.GetCraftChips()>=3)
				Success=true;
