...
public class CECS274 extends Card{
	public CECS274(String Name, String Path, int Version,int[]CR){
		CardName=Name;
		imagePath=Path;
	...
	public void PlayCard(Player currentPlayer,int ChipChoice,ArrayList<Card> Discard,ArrayList<Card> Deck,ArrayList<Room> RoomList){
		...
		int[] CorrectRooms=new int[]{9,10,17};
		
		for(int i=0; i<CorrectRooms.length;i++)
			if(TempPlayer.GetPlayerRoom().GetRoomID()==CorrectRooms[i]&&TempPlayer.GetLearningChips()>=7)
				Success=true;
